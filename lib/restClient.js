...
function filterQuery(obj) {
    ...
    Object.keys(obj).forEach(function (x) {
        if (typeof obj[x] === 'string') result[x] = { '$regex': obj[x] };else result[x] = obj[x];
    });
    if (Object.keys(result).length > 0) return stringify(result);
    return null;
}
...
exports.default = function (parseConfig) {
    ...
    var convertRESTRequestToHTTP = function convertRESTRequestToHTTP(type, resource, params) {
        ...
        switch (type) {
            case _types.GET_LIST:
                {
                    ...
                    var field = params.sort && params.sort.field != null ? params.sort.field : 'createdAt';
            ...
            case _types.GET_MANY:
                {
                    var _query = {
                        where: stringify({ 'objectId': { '$in': params.ids } })
                    };
            ...
            case _types.GET_MANY_REFERENCE:
                {
                    ...
                    var _field = params.sort && params.sort.field != null ? params.sort.field : 'createdAt';
                    var _order = params.sort && params.sort.order != null ? params.sort.order : 'ASC';
                    var _query2 = {
                        order: _order === 'DESC' ? '-' + _field : _field,
                        limit: _perPage,
                        skip: (_page - 1) * _perPage,
                        where: stringify(_defineProperty2({}, params.target, params.id))
                    };
    ...
    var convertHTTPResponseToREST = function convertHTTPResponseToREST(response, type, resource, params) {
        ...
        switch (type) {
            ...
            case _types.GET_MANY_REFERENCE:
                return {
                    data: json.results.map(function (x) {
                        return _extends2({}, x, { id: x.objectId });
                    }),
                    total: json.count
                };
            case _types.CREATE:
            ...
            case _types.DELETE:
                return {
                    data: _extends2({}, json, { id: json.objectId })
                };
            case _types.GET_MANY:
                return {
                    data: json.results.map(function (x) {
